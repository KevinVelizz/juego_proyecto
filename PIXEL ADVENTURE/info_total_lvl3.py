...
class DataLevelTres:
    def __init__(self,screen) -> None:        
        self.lista_info = cargar_json_data("PIXEL ADVENTURE/levels.json")[0]["level_three"]
        self.screen = screen
        self.lista_enemy = []
        self.lista_platform = []
        self.list_fruits = []
    ...
    def create_ememies(self):
        for enemy in self.lista_info["enemys"]:
            for i in range(enemy["amount"]):
                if(enemy["name"] == "ghost"):
                    self.lista_enemy.append(EnemyGhost(enemy["position_x"][i],enemy["position_y"][i],enemy["speed"],50,50,self.screen))
                if(enemy["name"] == "planta"):
                    self.lista_enemy.append(EnemyPlant(enemy["position_x"][i],enemy["position_y"][i],enemy["speed"],50,50,self.screen))

    def create_platforms(self):
        for platform in self.lista_info["platforms"]:
    ...
    def update(self,delta_ms,player):
        ...
        for fruit in self.list_fruits:
            fruit.update(delta_ms,self.lista_platform,player.rect_ground_collition)
