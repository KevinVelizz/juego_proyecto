...
class DataLevelDos:
    def __init__(self,screen) -> None:        
        self.lista_info = cargar_json_data("PIXEL ADVENTURE/levels.json")[0]["level_two"]
        self.screen = screen
        self.lista_enemy = []
        self.lista_platform = []
        self.list_fruits = []
    ...
    def create_ememies(self):
        for enemy in self.lista_info["enemys"]:
            if(enemy["name"] == "ghost"):
                self.lista_enemy.append(EnemyGhost(enemy["position_x"],enemy["position_y"],enemy["speed"],50,50,self.screen))
            if(enemy["name"] == "rino"):
                self.lista_enemy.append(EnemyRino(enemy["position_x"],enemy["position_y"],enemy["speed"],50,50,self.screen))
